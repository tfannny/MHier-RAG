...
class PageTextPreparation:
    ...
    def process_reports(
        ...
        if reports_dir: #/01_parsed_reports
    ...
    def _apply_formatting_rules(self, blocks):
        ...
        while i < n:
            block = blocks[i]
            block_type = block.get("type") #caption
            ...
            if block_type == "picture": 
                group_blocks = []
                picture_block = block
                group_blocks.append(picture_block)
                group_text = self._render_picture_group(group_blocks) 
    ...
    def _render_picture_group(self, group_blocks):
        chunk = []
        for blk in group_blocks:
            ...
            elif blk_type == "picture":
                picture_id = blk.get("picture_id")
                if picture_id is None:
                    continue
                picture_content = self._get_picture_by_id(picture_id) 
                chunk.append(f"{picture_content['description']}\n")
    ...
    def _get_picture_by_id(self, picture_id):
        for t in self.report_data.get("pictures", []):
            if t.get("picture_id") == picture_id:
                return t.get("semantics", "") 
        raise ValueError(f"Picture with ID={picture_id} not found in report_data!")
